...
def normalize(source: str, row: dict[str, Any]) -> JobListing:
    ...
    external = str(row.get("id") or row.get("slug") or row.get("id") or url or f"{company}:{title}")
    external_id = hashlib.sha256(f"{source}:{external}".encode()).hexdigest()
    return JobListing(external_id=external_id, company=company, title=title, location=row.get("candidate_required_location") or row.get("location") or "Remote", remote=True, salary=row.get("salary"), description=row.get("description"), url=url, source=source)
    ...
    async with httpx.AsyncClient(timeout=20, follow_redirects=True, headers={"User-Agent": "MACCY-CREATIONS/1.0"}) as client:
        for source, endpoint in PUBLIC_SOURCES.items():
            try:
                ...
                data = response.json()
                rows = data.get("jobs", data.get("data", data)) if isinstance(data, dict) else data
